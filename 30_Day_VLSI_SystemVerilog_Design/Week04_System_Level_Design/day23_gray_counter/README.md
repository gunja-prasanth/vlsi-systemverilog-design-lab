...
Binary counting may be change multiple bits during a single increment.

Example:
0011 → 0100 (3 bits change)

Gray code ensures the only one bit changes per transition.
...
- Write pointer and the Read pointer operate in different clock domains.
- Pointer values are synchronized across domains using flip-flop synchronizers.

If binary is used:
Multiple bit transitions may cause the metastability and incorrect pointer sampling.
